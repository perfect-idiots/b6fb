...
class CommentManager extends LoginDoubleChecker {
  ...
  public function getUnknownCommentsByGame(array $knownComments, string $game, bool $filter = false, bool $hidden = false): array {
    $serialized = implode(',', $knownComments);

    $dbResult = $this
      ->get('db-query-set')
      ->get('get-unknown-comments-by-game')
      ->executeOnce([$serialized, $game, (int) $filter, (int) $hidden], 9)
      ->fetch()
    ;
